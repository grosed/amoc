...
Rcpp::List amoc_dp_var1(const arma::mat& X_futu, const arma::mat& X_curr, const arma::vec& lambda, int delta, double eps)
{
  ...
  arma::vec bestvalue = arma::zeros<arma::vec>(n+1);
  arma::vec partition = arma::zeros<arma::vec>(n+1);
  arma::vec score = arma::zeros<arma::vec>(n+1);
  for(int l = 2; l < n; ++l)
    {
      
      score(l-1) = Rcpp::as<double>(rcpp_error_pred_seg_VAR1(X_futu, X_curr, 1, l, lambda, delta, eps)["MSE"]) +
	           Rcpp::as<double>(rcpp_error_pred_seg_VAR1(X_futu, X_curr, l+1, n, lambda, delta, eps)["MSE"]);
    }
